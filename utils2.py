...
def Optimize(func, 
    ...
    if initial_values is not None:
        if isinstance(initial_values, (list, torch.Tensor, np.ndarray)):
            # C'est une liste de valeurs, utilisez-la directement
            init_values = torch.tensor(initial_values, dtype=torch.float64)
        else:
            # Traitement existant pour un nom de fichier
...
# ---------- PREPARE DATA FUNCTIONS for single


def prepare_data_level0_single(df, year):
    # Création des tenseurs à partir du dataframe avec gestion des types
    tensor_vars = {}
    for col in df.columns:
        if not pd.api.types.is_object_dtype(df[col]):
            try:
                # Convertir explicitement en float64 pour éviter les erreurs de type
                tensor_vars[col] = torch.tensor(df[col].values.astype(np.float64), dtype=torch.float64)
            except (TypeError, ValueError):
                print(f"Colonne ignorée: {col} - problème de conversion")

    # Identification des indices pour les modes sans voiture
    if year > 2016:
        level0_idx = torch.where(torch.isin(tensor_vars['TRANS'], torch.tensor([2., 3., 4., 6.])))[0]
    else:
        level0_idx = torch.where(torch.isin(tensor_vars['TRANS'], torch.tensor([2., 3., 5.])))[0]

    # Tous les indices valides
    all_idx = torch.where(~torch.isnan(tensor_vars['VOIT']))[0]

    # Retour du dictionnaire avec les données nécessaires
    return {
            "df": df,
            "vars": tensor_vars,
...
def prepare_data_level1_single(df):
    # Création des tenseurs à partir du dataframe avec gestion des types
    tensor_vars = {}
    for col in df.columns:
        if not pd.api.types.is_object_dtype(df[col]):
    ...
    # Indices pour les célibataires avec voiture
    has_car_idx = torch.where(tensor_vars['VOIT'] >= 1)[0]
    ...
    return {
            "df": df,
            "vars": tensor_vars,
            "has_car_idx": has_car_idx,
            "all_idx": all_idx
    }

def prepare_data_level2_single(df):
    # Création des tenseurs à partir du dataframe avec gestion des types
    tensor_vars = {}
    for col in df.columns:
        if not pd.api.types.is_object_dtype(df[col]):
